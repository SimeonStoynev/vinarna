...
public class BottledWineStorageService {
    ...
    public ObservableList<BottledWineStorageModel> getAllByWineAndPeriod(WineTypeModel wine, LocalDate startDate, LocalDate endDate) {
        List<BottledWineStorage> storage = repository.getLatestAllByWineAndPeriod(wine.toEntity(), startDate, endDate);
        return FXCollections.observableList(
                storage.stream().map(b -> new BottledWineStorageModel(
                        b.getId(),
                        new BottleTypeModel(b.getBottle_type_id()),
